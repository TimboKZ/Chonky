...
export const getNonNil: <T>(
  array: Nilable<T[]>,
  index: number
) => Nullable<T> = (array, index) => {
...
export const setupListeners = () => {
  if (isNil(window)) {
    throw new Error(
      '[Chonky] `window` object was not found - Chonky might not work correctly. Are we running in' +
        ' the browser?'
    );
  }
