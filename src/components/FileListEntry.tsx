  ...
  public render() {
    ...
    if (isObject(file)) {
      if (isFunction(onFileSingleClick)) {
        wrapperProps.onSingleClick = (event: InputEvent) =>
          onFileSingleClick(file, displayIndex, event);
      }
      if (isFunction(onFileDoubleClick)) {
        wrapperProps.onDoubleClick = (event: InputEvent) =>
          onFileDoubleClick(file, displayIndex, event);
      }
    }
