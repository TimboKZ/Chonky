...
export type SingleFileActionHandler = <T extends FileData = FileData>(
  file: T,
  event?: InputEvent
) => ActionHandlerResult;
export type MultiFileActionHandler = <T extends FileData = FileData>(
  files: T[],
  event?: InputEvent
) => ActionHandlerResult;

export type InternalClickHandler = (
  file: FileData,
  fileIndex: number,
  event: InputEvent
) => void;
