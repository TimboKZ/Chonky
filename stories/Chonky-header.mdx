<div style={{ textAlign: 'center' }}>
    <br />
    <br />
    <img src="./Chonky_clear.png" alt="Chonky logo" style={{ width: 500 }} />
    <br />
    <a href="https://www.npmjs.com/package/chonky" target="_blank">
        <img alt="NPM package" src="https://img.shields.io/npm/v/chonky.svg" />
    </a>
    <a href="https://tldrlegal.com/license/mit-license" style={{ marginLeft: 5 }} target="_blank">
        <img alt="MIT license" src="https://img.shields.io/npm/l/chonky" />
    </a>
    <a href="https://www.npmjs.com/package/chonky" style={{ marginLeft: 5 }} target="_blank">
        <img alt="NPM downloads" src="https://img.shields.io/npm/dt/chonky" />
    </a>
    <a href="https://github.com/TimboKZ/Chonky" style={{ marginLeft: 5 }} target="_blank">
        <img alt="GitHub stars" src="https://img.shields.io/github/stars/TimboKZ/Chonky" />
    </a>
    <a href="https://discord.gg/HT4ttdQ" style={{ marginLeft: 5 }} target="_blank">
        <img alt="Chat on Discord" src="https://img.shields.io/discord/696033621986770957?label=Chat%20on%20Discord" />
    </a>
    <br />
    <br />
    <br />
</div>
