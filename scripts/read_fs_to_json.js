...
const dirToFsTree = (fileMap, dirPath, parentFile) => {
  return readDir(dirPath, parentFile).then(files => {
    const promises = [];
    for (const file of files) {
      ...
      if (file.isDir)
        promises.push(
          dirToFsTree(fileMap, path.join(dirPath, file.name), file)
        );
    }
...
const prepareFsJson = (rootPath, outPath, customRootFolderName) => {
  ...
  return readFile(rootPath, null)
    ...
    .then(() =>
      fs.writeFile(
        outPath,
        JSON.stringify({ rootFolderId: rootFile.id, fileMap }, null, 2)
      )
    )
...
const japanPicsDir = path.resolve(
  __dirname,
  '..',
  '..',
  '..',
...
Promise.resolve()
  ...
  .then(results => {
    ...
    const fileMap = {
      ...
      [foreverFolderId]: createFolder(
        foreverFolderId,
        'Files that load forever',
        rootId,
    ...
    fileMap[longNameFolderId] = createFolder(
      longNameFolderId,
    ...
    for (let i = 0; i < nestedFolderCount; ++i) {
      ...
      nestedChild = createFolder(
        `nested-folder-${nestedFolderCount - i}`,
        folderName,
        `nested-folder-${nestedFolderCount - i - 1}`,
        children
      );
      fileMap[nestedChild.id] = nestedChild;
    }
    console.log(nestedChild);
    fileMap[nestedFolderId] = createFolder(
      nestedFolderId,
      `${nestedFolderCount} nested folders`,
      rootId,
      [nestedChild.id]
    );
    ...
    return fs.writeFile(
      demoFsJson,
      JSON.stringify({ rootFolderId: rootId, fileMap }, null, 2)
    );
  })
  .catch(console.error);
