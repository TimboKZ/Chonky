...
test('Double click calls onFileOpen(...)', async () => {
  ...
  const fileBrowser = mount<FileBrowser>(
    <FileBrowser files={demoFiles} onFileOpen={openSpy} />
  );
...
test('onOpenFiles(...) called after onFileOpen(...)', async () => {
  ...
  [mainFile, inputEvent] = singleOpenSpy.getCall(0).args as [
    FileData,
    InputEvent
  ];
  [selectedFiles, inputEvent] = multiOpenSpy.getCall(0).args as [
    FileData[],
    InputEvent
  ];
  assert.equal(inputEvent.type, InputEventType.Keyboard);
